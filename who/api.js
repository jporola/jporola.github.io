...
const filter = array => {
  return array.filter(o => o.id && o.avatar && o.name).map(o => {
    return {
      id: o.id,
      name: o.name,
      avatar: o.avatar
...
export const getPersons = async () => {
  const url =
    'https://2umdflpqk3.execute-api.us-west-2.amazonaws.com/prod/persons'
  const options = {
    headers: {
...
export const login = async (user, pass) => {
  const url =
    'https://2umdflpqk3.execute-api.us-west-2.amazonaws.com/prod/login'
  const data = {
    email: user,
    password: pass
  }
